...
class mFields {
	...
	function setCoresValue( $link_name, $link_desc, $address, $city, $state, $country, $postcode, $contactperson, $mobile, $date, $year, $telephone, $fax, $email, $website, $price, $link_hits, $link_votes, $link_rating, $link_featured, $link_created, $link_modified, $link_visited, $publish_up, $publish_down, $metakey, $metadesc, $user_id, $username ) {
...
class mFieldType_number extends mFieldTypeNumber {
	...
	function getOutput($view=1) {
		...
		return number_format((float)$this->getValue(), $decimals, $decimalPoint, $thousandSeparator);
	}
	...
	function getInputValue($arg=null)
	{
		...
		{
			return number_format($this->getValue(), $decimals, $decimalPoint, $thousandSeparator);
		}
...
class mFieldType_date extends mFieldType {
	...
	function getInputHTML() {
		...
		$html .= JHtml::_(
			...
			$this->getInputFieldName(), 
			'%Y-%m-%d',
            ($this->isRequired() ? array('required'=>true):null)
			);
