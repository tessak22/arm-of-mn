...
function editlink( $link_id, $cat_id, $for_approval=false, $option ) {
	...
	$fields = new mFields();
	$fields->setCoresValue( $row->link_name, $row->link_desc, $row->address, $row->city, $row->state, $row->country, $row->postcode, $row->contactperson, $row->mobile, $row->date, $row->year, $row->telephone, $row->fax, $row->email, $row->website, $row->price, $row->link_hits, $row->link_votes, $row->link_rating, $row->link_featured, $row->link_created, $row->link_modified, $row->link_visited, $row->publish_up, $row->publish_down, $row->metakey, $row->metadesc, $row->user_id, $row->owner );
...
function savelink( $option, $post=null ) {
	...
	$date = JFactory::getDate($row->publish_up, $tz);
