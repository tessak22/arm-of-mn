...
class RSEvent
{
	...
	protected function repeatevents($row) {
		...
		list($repeat_end,$repeat_end_time) = explode(' ',$row->repeat_end,2);
		...
		if (!empty($row->exclude_dates)) {
			$registry = new JRegistry;
			$registry->loadString($row->exclude_dates);
			if ($exclude = $registry->toArray()) {
				foreach ($exclude as $k => $exclude_date) {
					...
					$exclude[$k] = $exclude_date->format('Y-m-d H:i:s');
				}
