...
class rseventsproController extends JControllerLegacy
{
	...
	public function payment() {
		...
		if ($details->state == 1) {
			...
			return $this->setRedirect(rseventsproHelper::route('index.php?option=com_rseventspro&layout=show&id='.rseventsproHelper::sef($event->id, $event->name)));
		}
	...
	public function image() {
		...
		if ($event = $db->loadObject()) {
			...
			if ($data = $cache->call(array('rseventsproHelper', 'createImage'), $event, $width)) {
