...
RSEventsPro.Event = {
	...
	init: function () {
		...
		jQuery('#rsepro-recurring-events-show').on('click', function() {
			...
			jQuery('#rsepro-recurring-events').slideToggle(400, function() {
				if (jQuery(recEl).find('i').hasClass('icon-arrow-down')) {
					jQuery(recEl).find('i').removeClass('icon-arrow-down').addClass('icon-arrow-up');
				} else {
					jQuery(recEl).find('i').removeClass('icon-arrow-up').addClass('icon-arrow-down');
				}
			});
		});
		...
		// Enable/Disable registraion
		jQuery('#jform_registration').on('click', function() {
			RSEventsPro.Event.enableRegistraion();
		});
	...
	addFile: function() {
		...
		var i			= jQuery('<i>', { 'class' : 'icon-remove'});
	...
	saveLocation: function() {
		...
		jQuery.ajax({
			...
			if (response != 0) {
				jQuery('#rsepro-save-location').html('<i class="icon-ok"></i> ' + Joomla.JText._('COM_RSEVENTSPRO_SAVED'));
	...
	addTicket: function() {
		...
		jQuery.ajax({
			...
			jQuery('<li style="display:block;" class="rsepro-hide"><a data-toggle="tab" data-target="#rsepro-edit-ticket' + response.id + '" href="javascript:void(0);">' + tName + '</a></li>').insertBefore(jQuery('ul li a[data-target="#rsepro-edit-tab6"]').parent());
	...
	validate: function() {
		...
		// Check for end date, only when All day event option is not enable
		if (!jQuery('#jform_allday').is(':checked')) {
		...
		// Check if the end registration date occurs after the start registraion date
		if (jQuery('#jform_registration').is(':checked')) {
