(function($) {
	...
	$.fn.rsjoomlamap = function(options) {
				...
				success: function(json) {
					try {
						...
						$(json).each(function(i, element) {
							base.createMarker(element.id, element.coords, element.content);
						});
					} catch(e) {}
		...
		// Create marker
		base.createMarker = function(id, coords, content) {
			var marker = new google.maps.Marker({
				position: base.createLatLng(coords),
				map: base.map,
			});
			
			google.maps.event.addListener(marker, 'click', function() {
				base.infoWindow.setContent(content);
