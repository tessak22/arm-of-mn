		...
		if( $my->id == 0 )
		{
			...
			JFactory::getApplication('site')->redirect(
				JRoute::_(
					...
					. base64_encode(
						JRoute::_('index.php?option=com_mtree&task=' . $task . '&link_id='.$link_id)
					)
				),
				JText::sprintf( 'JGLOBAL_YOU_MUST_LOGIN_FIRST' )
			);

			// Show 404 if user is loggedin and not authorised.
		} else {
...
if( $mtconf->get('load_css') && $document->getType() == 'html')
{
	...
	if ( file_exists( 'components/com_mtree/templates/'.$mtconf->get('template') . '/template.css' ) )
	{
		$document->addStylesheet('components/com_mtree/templates/' . $mtconf->get('template') . '/template.css');
			...
			$document->addStylesheet('components/com_mtree/templates/kinabalu/template.css');
	}
...
function search( $option ) {
	...
	# Construct WHERE
	$link_fields = array('link_name', 'link_desc', 'address', 'city', 'postcode', 'state', 'country', 'email', 'website', 'contactperson', 'mobile', 'date', 'year', 'telephone', 'fax', 'metakey', 'metadesc', 'price' );
	...
	if($link === false)	{
		...
		if( method_exists($document,'addCustomTag') )
		{
			...
			if( $mtconf->get('use_twitter_card') && $mtconf->get('twitter_card_type') != '' && !empty($link->link_image) )
			{
				...
				if (!$mtconf->get('use_open_graph_protocol'))
				{
					$document->addCustomTag( '<meta name="twitter:title" content="'.htmlspecialchars($link->link_name).'"/>' );
...
function editlisting( $link_id, $option ) {
	...
	$fields = new mFields();
	$fields->setCoresValue( $link->link_name, $link->link_desc, $link->address, $link->city, $link->state, $link->country, $link->postcode, $link->contactperson, $link->mobile, $link->date, $link->year, $link->telephone, $link->fax, $link->email, $link->website, $link->price, $link->link_hits, $link->link_votes, $link->link_rating, $link->link_featured, $link->link_created, $link->link_modified, $link->link_visited, $link->publish_up, $link->publish_down, $link->metakey, $link->metadesc, $link->user_id, '' );
	...
	if( $mtconf->get('allow_imgupload') && $mtconf->get('images_per_listing') > 0 ) {
		$document->addScript( ltrim($mtconf->get('relative_path_to_js'),'/') . 'jquery-ui-1.8.24.custom.min.js');
	}

	if( $mtconf->get('use_map') == 1 )
	{
		$document->addScript( ltrim($mtconf->get('relative_path_to_js'),'/') . 'map.js');
	}
	...
	if(
		...
		($link_id > 0 && $my->id <> $link->user_id)
		||
		// No listing modification is allowed
...
function savelisting( $option ) {
	...
	if ( $mtconf->get('user_allowmodify') == 0 && $link_id > 0 )
	{
	...
	else
	{
		...
		foreach($fieldtype AS $ft) {
			...
			if( $ft->required_field == 1 && in_array($ft->cf_id,$assigned_cf_ids) && !$ft->hidden)
			{
